...
function parseMp3FrameHeader(header: number): Mp3FrameInfo | null {
    ...
    const sampleRateTables: Record<number, [number, number, number]> = {
        1: [44100, 48000, 32000],
        2: [22050, 24000, 16000],
        25: [11025, 12000, 8000],
    };
    const sampleRateKey = version === 2.5 ? 25 : (version as number);
    ...
    return {
        version, versionName: `MPEG-${version === 1 ? "1" : version === 2 ? "2" : "2.5"}`,
        layer, sampleRate, bitrate, channels: channelMode === 3 ? 1 : 2,
        frameSize: Math.floor((samplesPerFrame / 8 * bitrate * 1000) / sampleRate) + padding, samplesPerFrame,
    };
}
...
function parseMp3XingHeader(view: DataView, offset: number, frameInfo: Mp3FrameInfo) {
    ...
    return { codecMode: "VBR (Xing)", totalFrames, duration, bitrateKbps: avgBitrate };
}
...
function parseMp3VbriHeader(view: DataView, offset: number, frameInfo: Mp3FrameInfo) {
    ...
    return { codecMode: "VBR (VBRI)", totalFrames, duration, bitrateKbps };
}
...
function parseMp3Metadata(buffer: ArrayBuffer): ParsedAudioMetadata {
    ...
    for (let offset = startOffset; offset <= view.byteLength - 4; offset++) {
        ...
        if (frameInfo) {
            ...
            return {
                fileType: "MP3", sampleRate: frameInfo.sampleRate, channels: frameInfo.channels,
                bitsPerSample: 16, totalSamples: duration > 0 ? Math.floor(duration * frameInfo.sampleRate) : 0,
                duration, codecMode: vbrInfo?.codecMode ?? "CBR", bitrateKbps, totalFrames, codecVersion: frameInfo.versionName,
            };
        }
    }
...
function parseAacMetadata(buffer: ArrayBuffer): ParsedAudioMetadata {
    ...
    for (let offset = 0; offset <= data.length - 7; offset++) {
        ...
        return { fileType: "AAC", sampleRate, channels: channels || 2, bitsPerSample: 16, totalSamples: 0, duration: 0 };
    }
...
const MP4_CONTAINER_TYPES = new Set(["moov", "trak", "mdia", "minf", "stbl", "edts", "dinf", "udta", "ilst", "meta", "stsd", "wave"]);
...
function parseM4aMetadata(buffer: ArrayBuffer): ParsedAudioMetadata {
        ...
        while (offset + 8 <= end) {
            ...
            if (box.type === "mdhd" && contentStart + 24 <= boxEnd) {
                ...
                if (version === 0 && contentStart + 24 <= boxEnd) {
                    ...
                    if (timeScale > 0) { sampleRate = timeScale; duration = durationValue / timeScale; }
                ...
                if (!sampleRate) {
                    ...
                    if (fixedPointSampleRate > 0) sampleRate = Math.floor(fixedPointSampleRate / 65536);
                }
            }
    ...
    return { fileType: "M4A", sampleRate, channels, bitsPerSample, totalSamples: duration > 0 ? Math.floor(duration * sampleRate) : 0, duration };
}
...
function buildWindowCoefficients(size: number, windowFunction: SpectrumParams["windowFunction"]): Float32Array {
    ...
    if (size <= 1) { coeffs.fill(1); return coeffs; }
    for (let i = 0; i < size; i++) {
        switch (windowFunction) {
            case "hamming": coeffs[i] = 0.54 - 0.46 * Math.cos((2 * Math.PI * i) / (size - 1)); break;
            case "blackman": coeffs[i] = 0.42 - 0.5 * Math.cos((2 * Math.PI * i) / (size - 1)) + 0.08 * Math.cos((4 * Math.PI * i) / (size - 1)); break;
            case "rectangular": coeffs[i] = 1; break;
            case "hann": default: coeffs[i] = 0.5 * (1 - Math.cos((2 * Math.PI * i) / (size - 1))); break;
        }
    }
    return coeffs;
}
    ...
    for (let len = 2; len <= size; len <<= 1) {
        ...
        for (let i = 0; i < size; i += len) {
            ...
            for (let j = 0; j < half; j++) {
                const uReal = real[i + j]; const uImag = imag[i + j];
                const vReal = real[i + j + half] * wReal - imag[i + j + half] * wImag;
                const vImag = real[i + j + half] * wImag + imag[i + j + half] * wReal;
                real[i + j] = uReal + vReal; imag[i + j] = uImag + vImag;
                real[i + j + half] = uReal - vReal; imag[i + j + half] = uImag - vImag;
                const tempReal = wReal * wLenReal - wImag * wLenImag;
                wImag = wReal * wLenImag + wImag * wLenReal; wReal = tempReal;
            }
        }
    }
}
...
export async function analyzeSpectrumFromSamples(samples: Float32Array, sampleRate: number, params: SpectrumParams, onProgress?: AnalysisProgressCallback, shouldCancel?: AnalysisCancelCheck): Promise<SpectrumData> {
    ...
    for (let windowIndex = 0; windowIndex < numWindows; windowIndex += frameStride) windowIndices.push(windowIndex);
    if (windowIndices[windowIndices.length - 1] !== numWindows - 1) windowIndices.push(numWindows - 1);
    ...
    for (let i = 0; i < totalSlices; i++) {
        ...
        for (let j = 0; j < copyLen; j++) { real[j] = samples[start + j] * windowCoeffs[j]; imag[j] = 0; }
        for (let j = copyLen; j < fftSize; j++) { real[j] = 0; imag[j] = 0; }
        ...
        timeSlices[i] = { time: sampleRate > 0 ? start / sampleRate : 0, magnitudes };
        const currentPercent = Math.floor(((i + 1) / totalSlices) * 100);
        if (currentPercent > lastReportedPercent) { lastReportedPercent = currentPercent; reportProgress(onProgress, "spectrum", currentPercent, "Analyzing spectrum..."); }
        if ((i + 1) % 8 === 0) {
            const now = nowMs();
            if (now - lastYieldAt >= 16) { await nextTick(); lastYieldAt = nowMs(); throwIfCancelled(shouldCancel); }
        }
    }
    return { time_slices: timeSlices, sample_rate: sampleRate, freq_bins: freqBins, duration, max_freq: maxFreq };
}

function createAnalysisAudioContext(sampleRate: number): AudioContext {
    if (sampleRate > 0) { try { return new AudioContext({ sampleRate }); } catch { return new AudioContext(); } }
...
export async function analyzeAudioFile(file: File, params: SpectrumParams = DEFAULT_PARAMS, onProgress?: AnalysisProgressCallback, shouldCancel?: AnalysisCancelCheck): Promise<FrontendAnalysisPayload> {
    ...
    throwIfCancelled(shouldCancel);
    return analyzeAudioArrayBuffer({ fileName: file.name, fileSize: file.size, arrayBuffer }, params, (progress) => {
...
export async function analyzeAudioArrayBuffer(input: AudioArrayBufferInput, params: SpectrumParams = DEFAULT_PARAMS, onProgress?: AnalysisProgressCallback, shouldCancel?: AnalysisCancelCheck): Promise<FrontendAnalysisPayload> {
    ...
    try {
        ...
        return analyzeDecodedSamples(input, metadata, audioBuffer.getChannelData(0), params, onProgress, shouldCancel, audioBuffer.duration);
    } finally {
...
export async function analyzeDecodedSamples(input: AudioArrayBufferInput, metadata: ParsedAudioMetadata, samples: Float32Array, params: SpectrumParams = DEFAULT_PARAMS, onProgress?: AnalysisProgressCallback, shouldCancel?: AnalysisCancelCheck, durationOverride?: number): Promise<FrontendAnalysisPayload> {
    ...
    let peak = 0, sumSquares = 0, lastMetricsYieldAt = nowMs();
    for (let i = 0; i < samples.length; i++) {
        throwIfCancelled(shouldCancel); const sample = samples[i]; const absSample = Math.abs(sample);
        if (absSample > peak) peak = absSample; sumSquares += sample * sample;
        if ((i + 1) % METRICS_CHUNK_SIZE === 0 || i === samples.length - 1) {
            const now = nowMs();
            if (now - lastMetricsYieldAt >= 16) { await nextTick(); lastMetricsYieldAt = nowMs(); throwIfCancelled(shouldCancel); }
        }
    }
    const peakDB = peak > 0 ? 20 * Math.log10(peak) : -120;
    const rms = samples.length > 0 ? Math.sqrt(sumSquares / samples.length) : 0;
    const rmsDB = rms > 0 ? 20 * Math.log10(rms) : -120;
    const duration = durationOverride && durationOverride > 0 ? durationOverride : (metadata.duration > 0 ? metadata.duration : (analysisSampleRate > 0 ? samples.length / analysisSampleRate : 0));
    const totalSamples = metadata.totalSamples > 0 ? metadata.totalSamples : (duration > 0 ? Math.floor(duration * analysisSampleRate) : samples.length);
    const spectrum = await analyzeSpectrumFromSamples(samples, analysisSampleRate, params, (progress) => {
        const mappedPercent = 50 + (progress.percent * 0.45); reportProgress(onProgress, "spectrum", mappedPercent, progress.message);
    }, shouldCancel);
    return {
        result: {
            file_path: input.fileName, file_size: input.fileSize, file_type: metadata.fileType, sample_rate: analysisSampleRate,
            channels: analysisChannels, bits_per_sample: metadata.bitsPerSample, total_samples: totalSamples, duration,
            bit_depth: bitDepthLabel, dynamic_range: peakDB - rmsDB, peak_amplitude: peakDB, rms_level: rmsDB,
            codec_mode: metadata.codecMode, bitrate_kbps: metadata.bitrateKbps, total_frames: metadata.totalFrames, codec_version: metadata.codecVersion, spectrum,
        }, samples,
    };
}
