...
export function buildClickableArtists(
    artists: string,
    ...
    return names.map((name, index) => {
        ...
        if (artistData && (artistData.id || artistData.external_urls)) {
            return { id: artistData.id || '', name, external_urls: artistData.external_urls || '' };
        }
        if (names.length === 1) {
            return { id: fallbackArtistId || '', name, external_urls: fallbackArtistUrl || '' };
        }
