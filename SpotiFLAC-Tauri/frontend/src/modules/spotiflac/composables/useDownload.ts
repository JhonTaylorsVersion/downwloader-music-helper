...
import { downloadTrack, fetchSpotifyMetadata } from "../utils/api";
...
async function resolveTemplateISRC(
  settings: {
  ...
  try {
    return await invoke("get_track_isrc", { spotifyId: spotifyId });
  } catch {
    return "";
  }
}
export function useDownload(region: string) {
  ...
  const bulkDownloadType = ref<"all" | "selected" | null>(null);
  const downloadedTracks = ref<Set<string>>(new Set());
  const failedTracks = ref<Set<string>>(new Set());
  const skippedTracks = ref<Set<string>>(new Set());
  const currentDownloadInfo = ref<{ name: string; artists: string } | null>(
    null,
  );
  const shouldStopDownloadRef = ref(false);
  const downloadWithAutoFallback = async (
    ...
    const displayAlbumArtist =
      settings.useFirstArtistOnly && albumArtist
        ...
        : albumArtist;
    const resolvedTemplateISRC = await resolveTemplateISRC(
      settings,
      spotifyId || id,
    );
    ...
    if (trackName && artistName) {
      try {
        ...
        const existenceResults = await invoke<FileExistenceResult[]>(
          "check_files_existence",
          { outputDir, rootDir: settings.downloadPath, tracks: [checkRequest] },
        );
    ...
    if (!fileExists) {
      itemID = (await invoke("add_to_download_queue", {
        id: id,
        trackName: trackName || "",
    ...
    if (service === "auto") {
      ...
      if (spotifyId) {
        try {
          const urlsJson = (await invoke("get_streaming_urls", {
            spotifyId: spotifyId,
            region: region,
          })) as string;
    ...
    if (!singleServiceResponse.success && itemID) {
      await invoke("mark_download_item_failed", {
        itemId: itemID,
        error: singleServiceResponse.error || "Download failed",
      });
    }
    ...
    try {
      ...
      if (response.success) {
        if (response.already_exists) {
          ...
          skippedTracks.value.add(id);
          skippedTracks.value = new Set(skippedTracks.value);
        } else {
          toast.success(response.message);
        }
        downloadedTracks.value.add(id);
        downloadedTracks.value = new Set(downloadedTracks.value);
        failedTracks.value.delete(id);
        failedTracks.value = new Set(failedTracks.value);
      } else {
        toast.error(response.error || "Download failed");
        failedTracks.value.add(id);
        failedTracks.value = new Set(failedTracks.value);
      }
    } catch (err) {
  ...
  const handleDownloadSelected = async (
    ...
    bulkDownloadType.value = "selected";
    ...
    const existenceResults = await invoke<FileExistenceResult[]>(
      "check_files_existence",
      { outputDir, rootDir: settings.downloadPath, tracks: existenceChecks },
    );
    ...
    for (const id of selectedTracks) {
      ...
      const displayArtist =
        settings.useFirstArtistOnly && track.artists
          ? getFirstArtist(track.artists)
          : track.artists;
      const itemID = (await invoke("add_to_download_queue", {
        id: trackID,
    ...
    for (let i = 0; i < tracksToDownload.length; i++) {
      if (shouldStopDownloadRef.value) {
        toast.info(
    ...
    shouldStopDownloadRef.value = false;
    ...
    if (settings.createM3u8File && folderName) {
      try {
        ...
        await invoke("create_m3u8_file", {
          playlistName: folderName,
          outputDir: outputDir,
          filePaths: finalFilePaths.filter((p) => p !== ""),
        });
        toast.success("M3U8 playlist created");
      } catch (err) {
        logger.error(`failed to create m3u8 playlist: ${err}`);
  ...
  const handleStopDownload = () => {
    ...
    shouldStopDownloadRef.value = true;
